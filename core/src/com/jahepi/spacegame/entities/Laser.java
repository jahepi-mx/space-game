...
public class Laser extends AbstractEntity {
	...
	public Laser() {
		this.animation = new Animation(1.0f/10.0f, Resource.getInstance().getShipAtlas().findRegions("BlueTail"), Animation.PlayMode.LOOP);
		this.destroyAnimation = new Animation(1.0f/10.0f, Resource.getInstance().getShipAtlas().findRegions("BlueBulletExplo"), Animation.PlayMode.NORMAL);
		this.size.x = 1.5f;
		this.size.y = 5.0f;
	...
	public void setXY(float x, float y) {
		...
		this.rectangle.x = this.position.x - (size.x / 2);
	...
	public void render(SpriteBatch batch) {
		if (isDestroyed) {
			...
			batch.draw(region, position.x - (size.y / 2), position.y - (size.y / 2), size.x / 2, size.y / 2, size.y, size.x, 1.0f, 1.0f, 90.0f);
		}
	}
	...
	public boolean isOutOfBounds() {
		if (position.y > Config.GAME_HEIGHT) {
			return true;
		}
		return false;
	}
